...
/* 
* Matrix struct adapted from Stanford CS Theory 
* @ http://theory.stanford.edu/~arbrad/pfe/06/matrix.c
* Modified for arbitrary data type,
* Modified to use calloc instead of malloc to allocate the matrix
* Modified for Matrix to work on GPU
*/
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
 ...
 * Returns NULL if rows <= 0 or cols <= 0 or memory allocation failure
 * and otherwise a pointer to the new matrix.
 */
matrix * newMatrix(int rows, int cols) {
  ...
  // allocate a data_t array of length rows * cols
  data_t * mat_data = (data_t *) calloc(rows*cols,sizeof(data_t));
...
matrix * copyMatrix(matrix * mtx) {
  ...
  memcpy(cp->data, mtx->data, 
         mtx->rows * mtx->cols * sizeof(data_t));
 ...
 */
int setElement(matrix * mtx, int row, int col, data_t val) 
{
  if (!mtx) return -1;
  assert (mtx->data);
 ...
 * dimensions.
 */
int dotProduct(matrix * v1, matrix * v2, data_t * prod) {
